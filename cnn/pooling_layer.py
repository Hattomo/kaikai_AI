...
class Pooling_Layer:
    ...
    def pooling(self, train_data):
        channel = len(train_data)
        height = len(train_data[0])
        width = len(train_data[0][0])
            ...
            for i in range(out_height):
                for j in range(out_width):
                    partical_data = train_data[h][i * self.pooling_size[0]:(i+1) * self.pooling_size[0],
                                                  i * self.pooling_size[1]:(i+1) * self.pooling_size[1]]  #bug
    ...
    def backpooling(self, input_error):
